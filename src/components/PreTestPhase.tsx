...
// "แบบทดสอบก่อนเรียน" (Pre-test) — an individual, self-paced ASSESSMENT, not a game round.
//
...
// is room-synchronized. player.preTestAnswers.length IS the current question index, so a refresh,
...
export const PreTestPhase = ({ player, onAnswer }: PreTestPhaseProps) => {
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')

  // The student's own saved count is the resume point and the progress indicator, both.
  const answeredCount = player.preTestAnswers.length
  const finished = answeredCount >= ASSESSMENT_QUESTION_COUNT
  const currentQuestion = PRE_TEST_QUESTIONS[answeredCount]
  ...
  const submit = async (selectedChoiceId: string): Promise<void> => {
    ...
    try {
      ...
      // Advancing happens implicitly: the saved answer grows preTestAnswers, which moves the
  ...
  if (finished) {
    return (
      <section className="glass-panel w-full p-6 text-center sm:p-8" aria-live="polite">
        <p className="eyebrow">แบบทดสอบก่อนเรียน</p>
        <h1 className="mt-2 text-2xl font-semibold sm:text-3xl">ทำแบบทดสอบก่อนเรียนครบแล้ว</h1>
        <div className="waiting-banner mt-6">
          <span className="pulse-dot" aria-hidden="true" />
          <span>
            <strong>รอครูเริ่มกิจกรรมทบทวน</strong>
